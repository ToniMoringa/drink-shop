...
const ProductForm = ({ product, onSuccess, onCancel }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    category: product?.category || 'hot', price: product?.price || '', image: product?.image || ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess();
    } catch (err) { alert('Save failed: ' + err.message); } finally { setUploading(false); }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.form}>
      ...
      <div className={styles.formRow}>
        <div className={styles.formGroup}><label className={styles.label}>Category *</label><select value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} className={styles.input}><option value="hot">Hot</option><option value="cold">Cold</option></select></div>
      ...
      <div className={styles.actions}>
        ...
        <button type="submit" className={styles.submitBtn} disabled={uploading}><Save size={18} />{uploading ? 'Saving...' : product ? 'Update' : 'Add Drink'}</button>
      </div>
    </form>
  );
};
